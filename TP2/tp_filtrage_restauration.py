...
def noise(im, br):
    """Cette fonction ajoute un bruit blanc gaussier d'écart type br
...
def view_spectre(im, option=1, hamming=False):
    ...
    if hamming:
        ...
        imt = (
            (1 - np.cos(2 * pi * XX / (tx - 1)))
            * (1 - np.cos(2 * pi * YY / (ty - 1)))
            * imt
        )
...
def median_filter(im, typ=1, r=1, xy=None):
    ...
    si typ==3 alors xy est un couple de liste de x et liste de y
         ([-1,0,1] , [0,0,0]) donne un median sur un segment horizontal de taille trois.
    """
    lx = []
    ly = []
    (ty, tx) = im.shape
    ...
    for k in range(len(lx)):
        tab[k, :] = im[
            deby + ly[k] : deby + tty + ly[k], debx + lx[k] : debx + ttx + lx[k]
        ].reshape(-1)
    out = im.copy()
...
#     np.argsort(u, axis=None), u.shape
...
imRotBL = im
for i in range(8):
    imRotPPV = rotation(imRotPPV, 45, clip=True)
    imRotBL = rotation(imRotBL, 45, clip=True, ech = 1)
...
# %%
viewimage(rotation(im, 45, clip=True, alpha=0.5))
viewimage(rotation(im, 45, clip=True, ech = 1, alpha=0.5))
...
viewimage(im)
# print(var_image(im, 0, 0, 10, 10))
viewimage(imBr)
# print(var_image(imBr, 0, 0, 10, 10))
viewimage(filtre_lineaire(imBr, get_gau_ker(2)))
# print(var_image(filtre_lineaire(imBr, get_gau_ker(2)), 0, 0, 10, 10))
viewimage(median_filter(imBr, r=1))

# %%
im = skio.imread("images/pyra-impulse.tif")
viewimage(im)
viewimage(filtre_lineaire(im, get_gau_ker(2)))
viewimage(median_filter(im, r=1))
...
med = median_filter(imBr, typ = 2, r = 4)
varMed = var_image(med, 0, 0, 256, 256)
viewimage(med)

const = filtre_lineaire(imBr, get_cst_ker(2))
varConst = var_image(const, 0, 0, 256, 256)
viewimage(const)
