...
viewimage_color(
    im, normalise=False
)  # on ne normalise pas pour garder l'image comme a l'origine
...
im = skio.imread("images/maison.tif")
plt.hist(im.reshape((-1,)), bins=256)  # le reshape transforme en tableau 1D
plt.show()
...
plt.hist(im.reshape((-1,)), 256)
plt.show()

plt.hist(imbr.reshape((-1,)), 256)
plt.show()
...
histo = histo / histo.sum()
histocum = histo.cumsum()
plt.hist(im.reshape((-1,)), bins=256)
plt.show()
plt.plot(histocum)
plt.show()
...
(histo, bins) = np.histogram(
    255 * imequal.reshape((-1,)), np.arange(0, 256)
)  # le reshape est inutile pour np.histogram, mais on le laisse pour la compatibilite avec plt.hist
histo = histo / histo.sum()
histocum = histo.cumsum()
plt.hist(255 * imequal.reshape((-1,)), bins=256)
plt.show()
plt.plot(histocum)
plt.show()
...
# %% prescription d'histogramme constante
u = skio.imread("images/vue1.tif")
viewimage(u)

ushape = u.shape
uligne = u.reshape((-1,))
vligne = [i for i in range(256) for j in range(u.shape[0] * u.shape[1] // 256)]
ind = np.argsort(uligne)
unew = np.zeros(uligne.shape, uligne.dtype)
unew[ind] = vligne
unew = unew.reshape(ushape)
...
plt.plot(np.log(np.histogram(gradx(im), 256)[0]))
plt.show()
viewimage(gradx(im))
